...
setenv_arm7()
{
	...
	setenv_all
	rm CMakeCache.txt
	cmake  -G 'Unix Makefiles' -DCMAKE_TOOLCHAIN_FILE=./port/iOS/IPHONEOS_ARM7_TOOLCHAIN.cmake -DASSIMP_ENABLE_BOOST_WORKAROUND=ON -DASSIMP_BUILD_STATIC_LIB=ON
}
...
setenv_i386()
{
	...
	setenv_all
	rm CMakeCache.txt
	cmake  -G 'Unix Makefiles' -DCMAKE_TOOLCHAIN_FILE=./port/iOS/IPHONEOS_i386_TOOLCHAIN.cmake -DASSIMP_ENABLE_BOOST_WORKAROUND=ON -DASSIMP_BUILD_STATIC_LIB=ON
}
...
create_outdir()
{
	for lib_i386 in `find $BUILD_DIR/i386 -name "lib*\.a"`; do
		...
		lipo -arch -arch armv7 $lib_arm7 -arch i386 $lib_i386 -create -output $lib
	done
...
rm -rf $BUILD_DIR
mkdir -p $BUILD_DIR/arm7 $BUILD_DIR/i386
